 ...
 *   argv[5] = facts file path (host-side; written before exit).
 *   argv[6] = rootfs dir (host-side; bind-mounts happen inside user ns).
 *   argv[7] = workspace dir (host-side; bind-mounted into rootfs/workspace RW).
 *   argv[8] = binary to exec (after chroot+seccomp; chroot-relative path).
 *   argv[9..] = args for the binary.
 *
