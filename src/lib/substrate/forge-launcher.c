/*
 * Forge — Phase 18X: substrate launcher (forge-launcher.c)
 *
 ...
 * The launcher key NO LONGER lives on disk where the worker can read it. The
 * substrate supervisor (a TRUSTED process) reads the key into memory at
 * startup, DELETES the file, then writes the key PEM into an unlinked temp
 * file (anonymous fd) for each substrate invocation. The fd number is passed
 * to the launcher as argv[1] (e.g., "3"). The launcher reads the key from
 * that fd via fdopen(), parses the PEM, then CLOSES the fd so the key
 * material doesn't linger. The worker process never sees the fd, never sees
 * the key file path, never sees the key content.
 *
 * Invoked as:
 *   forge-launcher <launcher_key_fd> <nonce> <execution_id>
 *                  <substrate_instance_id> <worker_id> <repository_sha>
 *                  <facts_file> <rootfs_dir> <workspace_dir>
 *                  <binary> [args...]
 *
 *   argv[1] = launcher key fd number (e.g., "3"). The supervisor passes
 *             this as the child's fd 3 via Node.js spawn stdio option
 *             ['ignore', 'pipe', 'pipe', <keyFd>]. The launcher reads the
 *             key PEM from this fd, then closes it.
 *   argv[2] = nonce (bound into the launcher signature).
 *   argv[3] = execution_id (bound into the launcher signature).
 *   argv[4] = substrate_instance_id (UUID generated by runInSubstrate).
 *   argv[5] = worker_id (Phase 18Z-A — bound into the artifact manifest).
 *   argv[6] = repository_sha (Phase 18Z-A — bound into the artifact manifest).
 *   argv[7] = facts file path (host-side; written before exit).
 *   argv[8] = rootfs dir (host-side; bind-mounts happen inside user ns).
 *   argv[9] = workspace dir (host-side; bind-mounted into rootfs/workspace RW).
 *   argv[10] = binary to exec (after chroot+seccomp; chroot-relative path).
 *   argv[11..] = args for the binary.
 *
 ...
 *
 * Flow:
 *   1. Read launcher Ed25519 private key from the fd (BEFORE chroot — the fd
 *      is inherited from the supervisor; the key PEM is in the kernel's page
 *      cache for that fd). CLOSE the fd immediately after reading so the key
 *      material is not lingering in any fd table.
 ...
 *      launcherSignedAt }.
 *  16. Phase 18Z-A: Write the same JSON to /workspace/attestation.json (so the
 *      manifest can include the signed attestation as an artifact).
 *  17. Phase 18Z-A: Walk /workspace/logs/ + hash /workspace/attestation.json,
 *      build artifact manifest entries, sort by artifactId, build canonical
 *      manifest JSON, compute manifest hash (SHA-256), sign manifest hash with
 *      the launcher Ed25519 key (same key as step 14), write
 *      /workspace/manifest.json.
 *  18. Exit with the workload's exit code (or 128+signal if killed by signal).
 *
 ...
 * private key is read from the anonymous fd BEFORE chroot (the supervisor
 * passes the fd via stdio[3]) but the signing operation happens AFTER
 * chroot+seccomp, so the signature cryptographically attests to the
 * post-seccomp kernel state.
 *
 * Environment:
...
/* Phase 18Z-A: Artifact manifest limits + paths. Must match the TypeScript
 * constants in src/lib/artifact-manifest.ts. */
#define MAX_MANIFEST_ENTRIES 200
#define MANIFEST_ENTRY_ID_LEN 128
#define MANIFEST_ENTRY_TYPE_LEN 64
#define MANIFEST_ENTRY_PATH_LEN 256
#define MANIFEST_ENTRY_MEDIA_LEN 64
#define MANIFEST_ENTRY_STORAGE_LEN 256
#define MANIFEST_PATH "/workspace/manifest.json"
#define ATTESTATION_PATH "/workspace/attestation.json"
#define WORKSPACE_LOGS_DIR "/workspace/logs"
...
static EVP_PKEY *read_launcher_key_from_fd(int key_fd) {
  /* Phase 18X: read the launcher key PEM from an inherited anonymous fd.
   * The supervisor (Node.js) creates an unlinked temp file, writes the key
   * PEM to it, then passes the fd as the child's stdio[3]. We fdopen() it,
   * parse the PEM, CLOSE the fd (so the key material is gone from our fd
   * table), and return the parsed key. */
  FILE *kf = fdopen(key_fd, "r");
  if (!kf) {
    fprintf(stderr, "forge-launcher: fdopen(key_fd=%d) failed: %s\n",
            key_fd, strerror(errno));
    /* If fdopen failed, we still own the fd — close it. */
    close(key_fd);
    return NULL;
  }
  BIO *bio = BIO_new_fp(kf, BIO_CLOSE /* BIO closes the FILE* (and the fd) */);
  if (!bio) {
    fprintf(stderr, "forge-launcher: BIO_new_fp(key_fd=%d) failed\n", key_fd);
    fclose(kf);
    return NULL;
  }
  EVP_PKEY *pkey = PEM_read_bio_PrivateKey(bio, NULL, NULL, NULL);
  BIO_free(bio); /* also fclose's kf (BIO_CLOSE) — fd is now closed */
  if (!pkey) {
    fprintf(stderr,
            "forge-launcher: failed to parse launcher private key from fd %d "
...
/* ----------------------------------------------------------------------- */
/* Phase 18Z-A: Artifact manifest — entry classification + canonical JSON  */
/* ----------------------------------------------------------------------- */

struct manifest_entry {
  char artifactId[MANIFEST_ENTRY_ID_LEN];
  char type[MANIFEST_ENTRY_TYPE_LEN];
  char path[MANIFEST_ENTRY_PATH_LEN];
  char mediaType[MANIFEST_ENTRY_MEDIA_LEN];
  long long size;
  char sha256[HASH_HEX_LEN];   /* 64 hex + NUL */
  char storageRef[MANIFEST_ENTRY_STORAGE_LEN];
};

struct artifact_pattern {
  const char *filename_prefix;  /* exact match if exact==1, prefix match if 0 */
  int exact;
  const char *type;             /* ArtifactType string */
  const char *media_type;
};

/* Known log-file patterns → artifact types. Must match the TypeScript
 * ArtifactType union in src/lib/artifact-manifest.ts. */
static const struct artifact_pattern ARTIFACT_PATTERNS[] = {
  { "source-materialization.txt", 1, "source-materialization",    "text/plain"        },
  { "dependency-lockfile.json",   1, "dependency-lockfile",       "application/json"  },
  { "install.log",                1, "install-log",               "text/plain"        },
  { "build.log",                  1, "build-log",                 "text/plain"        },
  { "startup.log",                1, "startup-log",               "text/plain"        },
  { "runtime-stdout.log",         1, "runtime-stdout",            "text/plain"        },
  { "runtime-stderr.log",         1, "runtime-stderr",            "text/plain"        },
  { "crash-output.log",           1, "crash-output",              "text/plain"        },
  { "test-results.json",          1, "test-results",              "application/json"  },
  { "manifest-output.json",       1, "manifest-output",           "application/json"  },
  { "health-trace-",              0, "health-trace",              "application/json"  },
  { "api-journey-",               0, "api-journey-trace",         "application/json"  },
};
static const size_t NUM_ARTIFACT_PATTERNS =
    sizeof(ARTIFACT_PATTERNS) / sizeof(ARTIFACT_PATTERNS[0]);

/**
 * Classify a log filename → artifact type + media type.
 * Returns 1 if the filename matches a known pattern, 0 if unknown.
 * For prefix matches (health-trace-N.json), the artifactId is derived from
 * the filename without extension (e.g., "health-trace-0").
 */
static int classify_artifact(const char *filename,
                             char *type_out, size_t type_len,
                             char *media_out, size_t media_len,
                             char *artifact_id_out, size_t id_len) {
  for (size_t i = 0; i < NUM_ARTIFACT_PATTERNS; i++) {
    const struct artifact_pattern *p = &ARTIFACT_PATTERNS[i];
    if (p->exact) {
      if (strcmp(filename, p->filename_prefix) == 0) {
        snprintf(type_out, type_len, "%s", p->type);
        snprintf(media_out, media_len, "%s", p->media_type);
        snprintf(artifact_id_out, id_len, "%s", p->type);
        return 1;
      }
    } else {
      size_t plen = strlen(p->filename_prefix);
      if (strncmp(filename, p->filename_prefix, plen) == 0) {
        snprintf(type_out, type_len, "%s", p->type);
        snprintf(media_out, media_len, "%s", p->media_type);
        /* artifactId = filename without ".json" extension.
         * e.g., "health-trace-0.json" → "health-trace-0". */
        size_t flen = strlen(filename);
        size_t copy_len = flen;
        if (flen > 5 && strcmp(filename + flen - 5, ".json") == 0) {
          copy_len = flen - 5;
        }
        if (copy_len >= id_len) copy_len = id_len - 1;
        memcpy(artifact_id_out, filename, copy_len);
        artifact_id_out[copy_len] = '\0';
        return 1;
      }
    }
  }
...
/**
 * Hash a file's content. Opens the file, reads in chunks, updates a SHA-256
 * context, finalizes. Returns 0 on success, -1 on error.
 *
 * @param path    file path to hash (inside the chroot).
 * @param sha_hex_out  output buffer (>= HASH_HEX_LEN = 65 bytes).
 * @param size_out      output: file size in bytes.
 */
static int hash_file(const char *path, char *sha_hex_out, long long *size_out) {
  ...
  to_hex(digest, digest_len, sha_hex_out);
  *size_out = total;
  return 0;
}

/**
 * Comparison function for qsort — sort manifest entries by artifactId
 * (alphabetical). Ensures the canonical manifest JSON is deterministic
 * regardless of readdir order.
 */
static int compare_entries(const void *a, const void *b) {
  const struct manifest_entry *ea = (const struct manifest_entry *)a;
  const struct manifest_entry *eb = (const struct manifest_entry *)b;
  return strcmp(ea->artifactId, eb->artifactId);
}

/**
 * Append one manifest entry's canonical JSON to the open_memstream.
 * Entry keys MUST be sorted alphabetically: artifactId, mediaType, path,
 * sha256, size, storageRef, type. This matches the TypeScript
 * canonicalSerialize function in src/lib/artifact-manifest.ts.
 */
static void emit_entry_json(FILE *mem, const struct manifest_entry *e) {
  fprintf(mem, "{");
  fprintf(mem, "\"artifactId\":");
  json_escape(mem, e->artifactId);
  fprintf(mem, ",\"mediaType\":");
  json_escape(mem, e->mediaType);
  fprintf(mem, ",\"path\":");
  json_escape(mem, e->path);
  fprintf(mem, ",\"sha256\":");
  json_escape(mem, e->sha256);
  fprintf(mem, ",\"size\":%lld,", e->size);
  fprintf(mem, "\"storageRef\":");
  json_escape(mem, e->storageRef);
  fprintf(mem, ",\"type\":");
  json_escape(mem, e->type);
  fprintf(mem, "}");
}

/**
 * Build the canonical manifest JSON for hashing/signing.
 *
 * Top-level keys sorted: entries, executionId, repositorySha,
 * substrateInstanceId, workerId. Entries array is in sorted (by artifactId)
 * order. Each entry's keys are sorted (see emit_entry_json).
 *
 * EXCLUDES manifestHash + signature fields (those are derived FROM the hash).
 *
 * Returns malloc'd buffer (caller frees). Sets *out_len to byte length.
 */
static char *build_canonical_manifest_json(
    const struct manifest_entry *entries, size_t n_entries,
    const char *execution_id, const char *repository_sha,
    const char *substrate_instance_id, const char *worker_id,
    size_t *out_len) {
  ...
  for (size_t i = 0; i < n_entries; i++) {
    if (i > 0) fprintf(mem, ",");
    emit_entry_json(mem, &entries[i]);
  }
  fprintf(mem, "],\"executionId\":");
  json_escape(mem, execution_id);
  fprintf(mem, ",\"repositorySha\":");
  json_escape(mem, repository_sha);
  fprintf(mem, ",\"substrateInstanceId\":");
  json_escape(mem, substrate_instance_id);
  fprintf(mem, ",\"workerId\":");
  json_escape(mem, worker_id);
  fprintf(mem, "}");
...
/**
 * Write the full manifest JSON (with hash + signature + metadata) to a file.
 * The JSON is NOT canonical (it includes manifestHash + signature fields),
 * but the manifestHash field IS the SHA-256 of the canonical form (computed
 * separately).
 */
static int write_manifest_file(
    ...
    const char *execution_id, const char *repository_sha,
    const char *substrate_instance_id, const char *worker_id,
    const char *manifest_hash, const char *created_at,
    const char *sig_hex, const char *signed_at) {
  ...
  int rc = 0;
  rc |= fprintf(f, "{");
  rc |= fprintf(f, "\"executionId\":"); json_escape(f, execution_id);
  rc |= fprintf(f, ",\"repositorySha\":"); json_escape(f, repository_sha);
  rc |= fprintf(f, ",\"workerId\":"); json_escape(f, worker_id);
  rc |= fprintf(f, ",\"substrateInstanceId\":"); json_escape(f, substrate_instance_id);
  rc |= fprintf(f, ",\"entries\":[");
  for (size_t i = 0; i < n_entries; i++) {
    if (i > 0) fprintf(f, ",");
    emit_entry_json(f, &entries[i]);
  }
  rc |= fprintf(f, "]");
  rc |= fprintf(f, ",\"manifestHash\":"); json_escape(f, manifest_hash);
  rc |= fprintf(f, ",\"createdAt\":"); json_escape(f, created_at);
  rc |= fprintf(f, ",\"launcherSignature\":"); json_escape(f, sig_hex);
  rc |= fprintf(f, ",\"launcherAlgorithm\":\"%s\"", LAUNCHER_ALGORITHM);
  rc |= fprintf(f, ",\"launcherKeyId\":\"%s\"", LAUNCHER_KEY_ID);
  rc |= fprintf(f, ",\"launcherSignedAt\":"); json_escape(f, signed_at);
  rc |= fprintf(f, "}\n");
...
/**
 * Phase 18Z-A: Build the artifact manifest.
 *
 * Walks /workspace/logs/ + reads /workspace/attestation.json, hashes each
 * file, builds entries, sorts by artifactId, computes canonical manifest JSON,
 * hashes it, signs the hash with the launcher key, writes /workspace/manifest.json.
 *
 * Returns 0 on success, -1 on failure (fail-closed — the TS side treats a
 * missing manifest.json as manifest=null → production blocked).
 */
static int build_and_sign_manifest(
    EVP_PKEY *launcher_key,
    const char *execution_id, const char *repository_sha,
    const char *substrate_instance_id, const char *worker_id) {

  struct manifest_entry entries[MAX_MANIFEST_ENTRIES];
  size_t n_entries = 0;

  /* 1. Walk /workspace/logs/ directory. */
  DIR *dir = opendir(WORKSPACE_LOGS_DIR);
  if (dir) {
    struct dirent *de;
    while ((de = readdir(dir)) != NULL) {
      ...
      int n = snprintf(file_path, sizeof(file_path), "%s/%s",
                       WORKSPACE_LOGS_DIR, de->d_name);
      if (n < 0 || (size_t)n >= sizeof(file_path)) continue;

      /* Classify — skip unknown filenames. */
      char type[MANIFEST_ENTRY_TYPE_LEN];
      char media[MANIFEST_ENTRY_MEDIA_LEN];
      char artifact_id[MANIFEST_ENTRY_ID_LEN];
      if (!classify_artifact(de->d_name, type, sizeof(type),
                             media, sizeof(media),
                             artifact_id, sizeof(artifact_id))) {
        continue;
      }

      if (n_entries >= MAX_MANIFEST_ENTRIES) break;

      /* Hash the file. */
      char sha_hex[HASH_HEX_LEN];
      long long size = 0;
      if (hash_file(file_path, sha_hex, &size) != 0) {
        fprintf(stderr, "forge-launcher: warning — failed to hash %s: %s\n",
                file_path, strerror(errno));
        continue;
      }

      struct manifest_entry *e = &entries[n_entries++];
      snprintf(e->artifactId, sizeof(e->artifactId), "%s", artifact_id);
      snprintf(e->type, sizeof(e->type), "%s", type);
      snprintf(e->path, sizeof(e->path), "logs/%s", de->d_name);
      snprintf(e->mediaType, sizeof(e->mediaType), "%s", media);
      snprintf(e->sha256, sizeof(e->sha256), "%s", sha_hex);
      e->size = size;
      snprintf(e->storageRef, sizeof(e->storageRef), "logs/%s", de->d_name);
    }
    closedir(dir);
  }

  /* 2. Add /workspace/attestation.json as a substrate-attestation entry. */
  {
    char sha_hex[HASH_HEX_LEN];
    long long size = 0;
    if (hash_file(ATTESTATION_PATH, sha_hex, &size) == 0) {
      if (n_entries < MAX_MANIFEST_ENTRIES) {
        struct manifest_entry *e = &entries[n_entries++];
        snprintf(e->artifactId, sizeof(e->artifactId), "substrate-attestation");
        snprintf(e->type, sizeof(e->type), "substrate-attestation");
        snprintf(e->path, sizeof(e->path), "attestation.json");
        snprintf(e->mediaType, sizeof(e->mediaType), "application/json");
        snprintf(e->sha256, sizeof(e->sha256), "%s", sha_hex);
        e->size = size;
        snprintf(e->storageRef, sizeof(e->storageRef), "attestation.json");
      }
    } else {
      fprintf(stderr,
              "forge-launcher: warning — failed to hash %s: %s. "
              "Manifest will be missing the required substrate-attestation type "
              "(verifyArtifactManifest will reject it — fail-closed).\n",
              ATTESTATION_PATH, strerror(errno));
    }
  }

  if (n_entries == 0) {
    fprintf(stderr,
            "forge-launcher: no artifacts found in %s or %s — manifest will be "
            "empty (verifyArtifactManifest will reject it — fail-closed).\n",
            WORKSPACE_LOGS_DIR, ATTESTATION_PATH);
  }

  /* 3. Sort entries by artifactId (deterministic canonical form). */
  qsort(entries, n_entries, sizeof(struct manifest_entry), compare_entries);

  /* 4. Build canonical manifest JSON. */
  size_t canon_len = 0;
  char *canon = build_canonical_manifest_json(
      entries, n_entries, execution_id, repository_sha,
      substrate_instance_id, worker_id, &canon_len);
  if (!canon) {
    fprintf(stderr, "forge-launcher: build_canonical_manifest_json failed\n");
    return -1;
  }

  /* 5. Compute manifest hash = SHA-256(canon). */
  char manifest_hash[HASH_HEX_LEN];
  {
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();
    if (!ctx) { free(canon); return -1; }
    unsigned char digest[32];
    unsigned int digest_len = 0;
    if (EVP_DigestInit_ex(ctx, EVP_sha256(), NULL) != 1 ||
        EVP_DigestUpdate(ctx, canon, canon_len) != 1 ||
    ...
    EVP_MD_CTX_free(ctx);
    to_hex(digest, digest_len, manifest_hash);
  }

  /* 6. Sign manifest_hash with the launcher Ed25519 key. */
  char sig_hex[SIG_HEX_LEN];
  if (sign_canonical(launcher_key, manifest_hash, strlen(manifest_hash),
                     sig_hex) != 0) {
    fprintf(stderr, "forge-launcher: manifest Ed25519 sign failed\n");
    ERR_print_errors_fp(stderr);
    free(canon);
    return -1;
  }

  /* 7. Write /workspace/manifest.json. */
  char created_at[32];
  char signed_at[32];
  format_iso_utc(created_at, sizeof(created_at));
  format_iso_utc(signed_at, sizeof(signed_at));

  int rc = write_manifest_file(
      MANIFEST_PATH, entries, n_entries, execution_id, repository_sha,
      substrate_instance_id, worker_id, manifest_hash, created_at,
      sig_hex, signed_at);

  free(canon);
  if (rc != 0) {
    fprintf(stderr, "forge-launcher: write_manifest_file(%s) failed: %s\n",
...
int main(int argc, char **argv) {
  ...
  /* Phase 18Z-A: argv layout now includes worker_id + repository_sha
   * (after substrate_instance_id) so the launcher can build the artifact
   * manifest. These are bound into the manifest hash + signature. */
  if (argc < 11) {
    fprintf(stderr,
            "forge-launcher: usage: %s <launcher_key_fd> <nonce> <execution_id> "
            "<substrate_instance_id> <worker_id> <repository_sha> "
  ...
  /* Phase 18X: argv[1] is now the launcher key fd NUMBER (not a file path).
   * The supervisor passes the fd as the child's stdio[3] (fd 3). */
  const char *launcher_key_fd_str = argv[1];
  const char *nonce = argv[2];
  const char *execution_id = argv[3];
  const char *substrate_instance_id = argv[4];
  const char *worker_id = argv[5];             /* Phase 18Z-A */
  const char *repository_sha = argv[6];        /* Phase 18Z-A */
  const char *facts_path = argv[7];
  const char *rootfs_dir = argv[8];
  const char *workspace_dir = argv[9];
  /* argv[10] = binary; argv[11..] = args. */

  /* ------------------------------------------------------------- *
   * STEP 1: read launcher Ed25519 private key from the anonymous fd
   *         (BEFORE chroot — the fd is inherited from the supervisor).
   *         The fd is CLOSED inside read_launcher_key_from_fd (via
   *         BIO_CLOSE on the underlying FILE*) so the key material does
   *         not linger in our fd table.
   * ------------------------------------------------------------- */
  char *endp = NULL;
  errno = 0;
  long key_fd_l = strtol(launcher_key_fd_str, &endp, 10);
  if (errno != 0 || endp == launcher_key_fd_str || *endp != '\0' ||
      key_fd_l < 0 || key_fd_l > 1024) {
    fprintf(stderr,
            "forge-launcher: invalid launcher_key_fd '%s' (expected a small "
            "non-negative integer like 3)\n",
            launcher_key_fd_str);
    return 1;
  }
  int launcher_key_fd = (int)key_fd_l;
  EVP_PKEY *launcher_key = read_launcher_key_from_fd(launcher_key_fd);
  if (!launcher_key) {
    return 1;
  }
  ...
  if (child == 0) {
    ...
    /* Phase 18Z-A: argv[10] = binary (was argv[8] before worker_id +
     * repository_sha were inserted at positions 5 + 6). */
  ...
  /* NOTE: launcher_key is NOT freed here yet — Phase 18Z-A needs it to sign
   * the artifact manifest in step 17. It's freed after the manifest is
   * written (see "STEP 17" below). */
  ...
  /* NOTE: `canonical` is NOT freed yet — STEP 16 reuses it for
   * /workspace/attestation.json. It's freed after STEP 16. */

  /* ------------------------------------------------------------- *
   * STEP 16 (Phase 18Z-A): write /workspace/attestation.json with the SAME
   * signed facts JSON content. This file is the "substrate-attestation"
   * artifact — the launcher includes it in the manifest so the manifest
   * cryptographically binds the attestation.
   *
   * The content is the same as facts.json (the launcher's signed attestation
   * facts). The TS side reads facts.json to build the full SandboxAttestation;
   * the attestation.json artifact is the same content, persisted alongside
   * the manifest for content-addressed retrieval.
   * ------------------------------------------------------------- */
  {
    int att_fd = open(ATTESTATION_PATH, O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0600);
    if (att_fd < 0) {
      fprintf(stderr,
              "forge-launcher: WARNING — open(%s) failed: %s. The manifest "
              "will be missing the substrate-attestation type — "
              "verifyArtifactManifest will reject it (fail-closed).\n",
              ATTESTATION_PATH, strerror(errno));
    } else {
      if (write_facts_json(
              att_fd, user_ns, pid_ns, net_ns, mnt_ns, observed_seccomp_mode,
              cap_eff, cpu_rl.rlim_cur, as_rl.rlim_cur, nproc_rl.rlim_cur,
              nofile_rl.rlim_cur, fsize_rl.rlim_cur, created_at, nonce,
              execution_id, substrate_instance_id, workload_exit_code,
              workload_signal, sha_out.hex, sha_err.hex, canonical, sig_hex,
              signed_at) != 0) {
        fprintf(stderr,
                "forge-launcher: WARNING — write_facts_json(%s) failed: %s. "
                "The manifest will be missing the substrate-attestation type.\n",
                ATTESTATION_PATH, strerror(errno));
      }
      /* write_facts_json fdopen(dup(fd),"w") + fclose — the dup'd fd is
       * closed by fclose, but the ORIGINAL att_fd is still open. Close it. */
      close(att_fd);
    }
  }
  ...
  /* ------------------------------------------------------------- *
   * STEP 17 (Phase 18Z-A): build + sign the artifact manifest.
   *
   * Walks /workspace/logs/ + reads /workspace/attestation.json, hashes each
   * file, builds manifest entries (sorted by artifactId), computes canonical
   * manifest JSON, hashes it (SHA-256), signs the hash with the launcher
   * Ed25519 key, writes /workspace/manifest.json.
   *
   * The manifest is signed INSIDE the substrate (after chroot+seccomp) with
   * the same launcher key that signed the attestation. The control plane
   * verifies the manifest signature with the pinned launcher public key.
   *
   * If this step fails, we DON'T fail the launcher exit (the attestation is
   * still valid — the substrate ran). The TS side treats a missing
   * manifest.json as manifest=null → production blocked (fail-closed).
   * ------------------------------------------------------------- */
  (void)build_and_sign_manifest(
      launcher_key, execution_id, repository_sha,
      substrate_instance_id, worker_id);

  /* Now we can safely free the launcher key — both signatures (attestation
   * + manifest) are done. */
  EVP_PKEY_free(launcher_key);

  /* ------------------------------------------------------------- *
   * STEP 18: exit with workload's exit code
   * ------------------------------------------------------------- */
